...
def get_config_string_value(section, key, default_value):
    ...
    try:
        ...
        if config_value == "":
            logging.warning("[" + str(section) + "/" + str(key) + "] value is empty")
...
if log_loading:
    ...
    logging.info(
        "\tairflow_rest_api_plugin_http_token_header_name: "
...
apis_metadata = [
    ...
    },
    {
        ...
        "arguments": [
            ...
            {
                "name": "execution_date",
                "description": "The execution date of the DAG (Example: 2017-01-02T03:04:05)",
                "form_input_type": "text",
                "required": True,
                "cli_end_position": 3,
            },
    ...
    },
    {
        ...
        "arguments": [
            {
                "name": "set",
                "description": "Set a variable. Please enter both key and value",
                "form_input_type": "custom_input",
            ...
            {
                "name": "export",
                "description": "Export variables to JSON file",
                "form_input_type": "text",
                "required": False,
            },
            {
    ...
    {
        "name": "variables_batch",
        "description": "CRUD operations on variables in batch mode",
        "airflow_version": "1.7.1 or greater",
        "http_method": ["GET", "POST", "DELETE"],
        "post_body_description": "Variables - POST Body Element - REQUIRED",
        "arguments": [],
    },
    {
        "name": "connections",
        ...
        "arguments": [
            ...
            {
                "name": "conn_id",
                "description": "Connection id, required to add/delete a connection",
                "form_input_type": "text",
                "required": False,
            },
            {
                "name": "conn_uri",
                "description": "Connection URI, required to add a connection",
                "form_input_type": "text",
                "required": False,
            },
            {
    ...
    },
    {
        ...
        "arguments": [
            ...
            {
                "name": "include_adhoc",
                "description": "Include dags with the adhoc argument.",
                "form_input_type": "checkbox",
                "required": False,
            },
            {
    ...
    },
    {
        ...
        "arguments": [
            ...
            {
                "name": "run-duration",
                "description": "Set number of seconds to execute before exiting",
                "form_input_type": "text",
                "required": False,
            },
            {
    ...
    },
    {
        ...
        "arguments": [
            {
                "name": "set",
                "description": "Set a Pool. Please enter the pool name, slot count and description",
                "form_input_type": "custom_input",
                "fields": [
                    {"pool_name": "Pool Name"},
    ...
    },
    {
        ...
        "fixed_arguments": [
            {
                "name": "no_confirm",
                "description": "Do not request confirmation",
                "fixed_value": "",
            }
        ],
    },
    ...
    {
        "name": "get_dag",
        "description": "Get the contents of a DAG",
        "airflow_version": "None - Custom API",
        "http_method": ["GET"],
        "arguments": [
            {
...
def http_token_secure(func):
    def secure_check(arg):
        ...
        if airflow_expected_http_token:
            ...
            if (
                request.headers.get(
                    airflow_rest_api_plugin_http_token_header_name, None
                )
                != airflow_expected_http_token
            ):
                ...
                return REST_API_Response_Util.get_403_error_response(
                    base_response=base_response, output=warning_message
                )
        return func(arg)
...
class REST_API_Response_Util:
    ...
    def get_200_response(base_response, output=None, airflow_cmd=None, warning=None):
        ...
        return REST_API_Response_Util._get_final_response(
    ...
    def _get_error_response(base_response, error_code, output=None):
        ...
        return (
            REST_API_Response_Util._get_final_response(
                base_response=base_response,
                output=output,
...
class REST_API(get_baseview()):
    ...
    def api(self):
        ...
        if self.is_arg_not_provided(api):
            ...
            return REST_API_Response_Util.get_400_error_response(
                base_response, "API should be provided"
            )
        ...
        if api_metadata is None:
            ...
            return REST_API_Response_Util.get_400_error_response(
                base_response, "API '" + str(api) + "' was not found"
            )
        ...
        if dag_id is not None and dag_id not in dag_bag.dags:
            ...
            return REST_API_Response_Util.get_400_error_response(
                base_response, "The DAG ID '" + str(dag_id) + "' does not exist"
            )
        ...
        if api == "version":
            ...
            final_response = self.get_dag(base_response)
        ...
        elif api == "variables_batch":
            final_response = self.variables_batch(base_response)
        else:
    ...
    def execute_cli(self, base_response, api_metadata):
        ...
        for argument in api_metadata.get("arguments", []):
            if (
                argument.get("cli_end_position") is not None
                and argument["cli_end_position"] > largest_end_argument_value
            ):
        ...
        run_api_in_background_mode = (
            "background_mode" in api_metadata and api_metadata["background_mode"]
        )
        ...
        for argument in api_metadata["arguments"]:
            ...
            if argument_value is not None:
                ...
                if "cli_end_position" in argument:
                    logging.info(
                        "argument['cli_end_position']: "
                        + str(argument["cli_end_position"])
                    )
        ...
        for fixed_argument in api_metadata.get("fixed_arguments", []):
            ...
            logging.info(
                "fixed_argument_name: "
                + str(fixed_argument_name)
                + ", fixed_argument_value: "
                + str(fixed_argument_value)
            )
        ...
        return REST_API_Response_Util.get_200_response(
            base_response=base_response, output=output, airflow_cmd=airflow_cmd
        )
    ...
    def rest_api_plugin_version(self, base_response):
        ...
        return REST_API_Response_Util.get_200_response(
            base_response, rest_api_plugin_version
        )
    ...
    def deploy_dag(self, base_response):
        ...
        if (
            "dag_file" not in request.files or request.files["dag_file"].filename == ""
        ):  # check if the post request has the file part
            logging.warning("The dag_file argument wasn't provided")
            return REST_API_Response_Util.get_400_error_response(
                base_response, "dag_file should be provided"
            )
        ...
        if dag_file and dag_file.filename.endswith(".py"):
            ...
            dagfile = Path(save_file_path)
            Path(dagfile.parent).mkdir(parents=True, exist_ok=True)
            ...
            return REST_API_Response_Util.get_400_error_response(
                base_response, "dag_file is not a *.py file"
            )
        ...
        if not (pause and unpause):
            if pause or unpause:
                try:
                    ...
                    warning = (
                        "Failed to set the state (pause, unpause) of the DAG: " + str(e)
                    )
    ...
    # Custom Function for finding a filename based on the DAG id
    def __get_file_by_id(self, dag_id):
        from pathlib import Path
        ...
        for f in all_files:
            f = str(f)
            try:
                # import the DAG file so that we can get the DAG_ID to execute the command to delete it
                import imp
                from airflow.models.dag import DAG

                if f.endswith(".py"):
                    dag_file = imp.load_source("module.name", f)
                    for object_name in dir(dag_file):
                        obj = getattr(dag_file, object_name)
                        if isinstance(obj, DAG):
                            if obj.dag_id == dag_id:
                                return f

            except Exception:
                traceback.print_exc()

    # Custom Function to get a dag
    def get_dag(self, base_response):
        ...
        f = self.__get_file_by_id(dag_id)
        if f:
            try:
                contents = open(f).read()
            except:
                return REST_API_Response_Util.get_400_error_response(
                    base_response, "Could not read file '{}'".format(f)
                )
            return REST_API_Response_Util.get_200_response(
                base_response=base_response,
                output=contents,
            ...
            return REST_API_Response_Util.get_400_error_response(
                base_response,
                "No file found that identifies a DAG with id '{}'".format(dag_id),
            )
    ...
    def delete_dag(self, base_response):
        ...
        try:
            ...
            warning = "failed to remove dag file '{}'".format(f)
            logging.warning(warning)

        # run the delete cli command
        airflow_cmd_split = []
        airflow_cmd_split = ["airflow", "delete_dag", dag_id, "--yes"]
        cli_output = self.execute_cli_command(airflow_cmd_split)
        print(cli_output)

        return REST_API_Response_Util.get_200_response(
            base_response=base_response,
            output="DAG [{}] has been deleted".format(dag_id),
            warning=warning,
        )
    ...
    def refresh_dag(self, base_response):
        ...
        try:
            ...
            error_message = (
                "An error occurred while trying to Refresh the DAG '"
                + str(dag_id)
                + "': "
                + str(e)
            )
    ...
    def variables_batch(self, base_response):
        try:
            logging.info("Executing custom variables_batch function")
            if request.method == "GET":
                self.execute_cli_command(
                    ["airflow", "variables", "--export", "/tmp/vars.json"]
                )
                response_body = json.load(open("/tmp/vars.json"))
                return {"statuscode": 200, "variables": response_body}
            elif request.method == "POST":
                self.execute_cli_command(
                    ["airflow", "variables", "--export", "/tmp/vars.json"]
                )
                current_vars = json.load(open("/tmp/vars.json"))
                print("current vars: {}".format(current_vars))

                for key in current_vars:
                    self.execute_cli_command(["airflow", "variables", "--delete", key])
                    logging.info("Deleted variable: {}".format(key))
                body = request.json
                json.dump(body, open("/tmp/vars.json", "w"))
                self.execute_cli_command(
                    ["airflow", "variables", "--import", "/tmp/vars.json"]
                )
                return REST_API_Response_Util.get_200_response(
                    base_response=base_response, output="Variables imported"
                )
        except:
            traceback.print_exc()
            raise
    ...
    def refresh_all_dags(self, base_response):
        ...
        return REST_API_Response_Util.get_200_response(
            base_response=base_response, output="All DAGs are now up to date"
        )
